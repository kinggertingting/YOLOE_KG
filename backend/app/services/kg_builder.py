...
import pandas as pd
from app.config import KG_PATH, USEFUL_RELATIONS, TARGET_CLASSES
...
def clean_relation(x):
    m = {'AtLocation': 'atlocation', 'PartOf': 'partof', 'HasProperty': 'hasproperty', 'IsA': 'isa'}
...
def load_conceptnet_json():
    ...
    for cls, relations in kg.items():
        ...
        for rel_type, objects in relations.items():
            ...
            if rel_clean not in USEFUL_RELATIONS:
                continue
            for obj in objects:
...
def build_class_concepts(target_classes, df, top_k=30):
    rows = df[df["relation_clean"].isin(USEFUL_RELATIONS)]
    class_concepts = {}
...
def get_concept_data(target_classes=None):
    ...
    # Extended
    DEFAULT_CONTEXT = ["road","street","highway","intersection","sidewalk","lane","tree","vegetation","barrier","fence",
                       "wall","bridge","traffic cone","vehicle","driver","pedestrian","bicycle","motorcycle",
                       "pedestrian","cyclist","van","animal","crosswalk","intersection","parking lot","railway","barrier"]
    ALL_CONTEXT = sorted(set(target_classes) | set(DEFAULT_CONTEXT))
